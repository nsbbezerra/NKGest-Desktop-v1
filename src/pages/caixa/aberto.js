...
import { Table, Button, Tooltip, Row, Col, Modal, Input, Spin, Statistic } from 'antd';
...
class OpenCaixa extends Component {
    ...
    findCaixas = async () => {
        ...
        await api.get('/cashier/listOpen').then(response => {
    ...
    sendOpenCashier = async () => {
        ...
        await api.post('/cashier/open', {
            funcionario: idFunc, valueOpened: this.state.value
        }).then(response => {
            ...
            },
            {
                ...
                render: (id) => <>
                    <Tooltip placement='top' title='Movimentar Caixa'>
                        <Button icon="dollar" size='small' style={{ marginRight: 5 }} type='primary' onClick={() => this.handleAutenticate(id)}>Movimentar Caixa</Button>
                    </Tooltip>
                </>,
                width: '10%',
                align: 'center'
            ...
            <>
                ...
                <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'row' }}>
                        ...
                        <Button icon='check' type='primary' onClick={() => this.setState({ modalOpenCaixa: true })}>Abrir Caixa</Button>
                ...
                <Spin spinning={this.state.spinner} size='large'>
                    ...
                    <Table columns={columns} dataSource={this.state.caixas} size='small' style={{ marginTop: 15 }} rowKey={(cashier) => cashier._id} />
                ...
                <Modal
                    ...
                    footer={[
                        <Button key="back" icon='close' type='danger' onClick={() => this.setState({ modalOpenCaixa: false })}>
                            Cancelar
                        </Button>,
                        <Button key="submit" icon='check' type="primary" loading={this.state.loading} onClick={() => this.sendOpenCashier()}>
                            Abrir
                        </Button>,
                    ]}
                    width='25%'
