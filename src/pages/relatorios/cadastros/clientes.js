...
export default function RelatorioClientes() {
  ...
  function warning(title, message) {
    ...
    async function finder() {
        ...
        setLoading(true);
        await api.post('/report/listClients', {
            find: find
        }).then(response => {
        ...
        <body>
          ...
          <button class="btn-print" onclick="window.print()">IMPRIMIR</button>
            ...
            return `<tr>
                        ...
                        ${cli.restrict ? (`<td class="active2">Sim</td>`) : (`<td class="restrict2">Não</td>`)}
                      </tr>`
        }).join('')}
                </tbody>
              </table>
        ...
        <>
            ...
            <div style={{ marginBottom: 10, width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Card size='small'>
                    ...
                    {find === 1 && (
                        <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>Clientes Ativos</strong></p>
                    )}
                    {find === 2 && (
                        <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>Clientes Bloqueados</strong></p>
                    )}
                    {find === 3 && (
                        <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>Clientes Restritos</strong></p>
                    )}
                    {find === 4 && (
                        <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>Todos os Clientes</strong></p>
                    )}
            ...
            <Table pagination={{ pageSize: 10 }} columns={columns} dataSource={clients} size='small' style={{ marginTop: 15, marginBottom: 15 }} rowKey={(cli) => cli._id} />

            {totalClients && (
                <Descriptions layout="vertical" bordered size='small'>
                    <Descriptions.Item label="Clientes Ativos" span={3}>{totalActive}</Descriptions.Item>
                    <Descriptions.Item label="Clientes Bloqueados" span={3}>{totalBlock}</Descriptions.Item>
                    <Descriptions.Item label="Clientes Restritos" span={3}>{totalRestrict}</Descriptions.Item>
                    <Descriptions.Item label="Total de Clientes" span={3}>{totalClients}</Descriptions.Item>
                </Descriptions>
            )}
