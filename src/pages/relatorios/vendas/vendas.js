...
import { Icon, Table, Button, Card, Descriptions, Select, Modal, Spin, TreeSelect, Divider, Statistic, Row, Tooltip, Drawer } from 'antd';
...
export default function RelatorioVendas() {
    ...
    const [vendasPay, setVendasPay] = useState([]);
    const [find, setFind] = useState(null);
    const [totalVendasPay, setTotalVendasPay] = useState(null);
    const [totalVendasWait, setTotalVendasWait] = useState(null);
    ...
    async function finder() {
        ...
        setLoading(true);
        await api.post('/report/listSales', {
            find: find, client: clientId, func: vendedorId, mes: mes, ano: ano
        }).then(response => {
            setVendas(response.data.salesWait);
            setVendasPay(response.data.salesPay);
            setTotalVendasPay(response.data.calcPay);
            setTotalVendasWait(response.data.calcWait);
            setLoading(false);
    ...
    async function viewInfoSalePay(id) {
        const result = await vendasPay.find(obj => obj._id === id);
        await setProdutos(result.products);
        setDrawerProducts(true);
    }

    async function viewInfoSaleWait(id) {
...
<head>
    <title>Imprimir Relatório de Vendas</title>
...
<body>
    ...
    <button class="btn-print" onclick="window.print()">IMPRIMIR</button>
        ...
        <div class="content">
            ...
            <h2>RELATÓRIO DE VENDA DE PRODUTOS</h2>

            <h5>VENDA DE PRODUTOS PAGAS</h5>
            ...
            return `<tr>
                     ...
                     <td class="totBrut2">R$ ${vend.valueBruto}</td>
                     <td class="totLiqu2">R$ ${vend.valueLiquido}</td>
            ...
            <h5>VENDA DE PRODUTOS EM ABERTO</h5>
            ...
            <table class="table-resume" cellspacing='0'>

                <thead>
                  <tr>
                    <td class="values">TOTAL VENDA DE PRODUTOS: PAGO</td>
                    <td class="money">R$ ${totalVendasPay}</td>
                  </tr>
                  <tr>
                    <td class="values">TOTAL VENDA DE PRODUTOS: EM ABERTO</td>
                    <td class="money">R$ ${totalVendasWait}</td>
                  </tr>
                  <tr>
                    <td class="valueslast">TOTAL GERAL</td>
                    <td class="moneylast">R$ ${totalVendasPay + totalVendasWait}</td>
                  </tr>
                </thead>
...
<p class="page-break">PageBreak</p>
    ...
    const columnSalesPay = [
        ...
        },
        {
            ...
            key: '_id',
            render: (id) => <>
                <Tooltip placement='top' title='Vizualizar Produtos'>
                    <Button icon='zoom-in' type='primary' shape='circle' size='small' onClick={() => viewInfoSalePay(id)} />
                </Tooltip>
            </>,
            width: '6%',
            align: 'center'
    ...
    const columnSalesWait = [
        ...
        },
        {
            ...
            render: (id) => <>
                <Tooltip placement='top' title='Vizualizar Produtos'>
                    <Button icon='zoom-in' type='primary' shape='circle' size='small' onClick={() => viewInfoSaleWait(id)} />
                </Tooltip>
            </>,
            width: '6%',
            align: 'center'
            ...
            <Spin spinning={spinner} size='large'>
                ...
                <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Card size='small'>
                        ...
                        {find === 2 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do mês de ${mes} do ano ${ano}`}</strong></p>
                        )}
                        {find === 3 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do ano ${ano}`}</strong></p>
                        )}
                        {find === 4 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do mês atual ao cliente: ${clientName}`}</strong></p>
                        )}
                        {find === 5 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do mês de ${mes} do ano ${ano}, ao cliente: ${clientName}`}</strong></p>
                        )}
                        {find === 6 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do ano ${ano}, ao cliente: ${clientName}`}</strong></p>
                        )}
                        {find === 7 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do mês atual pelo vendedor(a): ${vendedorName}`}</strong></p>
                        )}
                        {find === 8 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do mês de ${mes} do ano ${ano}, pelo vendedor(a): ${vendedorName}`}</strong></p>
                        )}
                        {find === 9 && (
                            <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Vendas de Produtos do ano ${ano}, pelo vendedor(a): ${vendedorName}`}</strong></p>
                        )}
                    ...
                    <>
                        <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10 }} bordered={false}>
                            <Row>
                                ...
                                <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>VENDAS PAGAS</span>
                            </Row>
                        </Card>

                        {!!vendasPay.length && (
                            <Table title={() => <p style={{ width: '100%', textAlign: 'center', fontWeight: 'bold', marginBottom: -4 }}>VENDA DE PRODUTOS</p>} pagination={{ pageSize: 10 }} columns={columnSalesPay} dataSource={vendasPay} size='small' rowKey={(cli) => cli._id} />
                        )}

                        <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10 }} bordered={false}>
                            <Row>
                                <Icon type='shopping-cart' style={{ fontSize: 20, color: '#fff', marginRight: 15 }} />
                                <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>VENDAS EM ABERTO</span>
                            </Row>
                        </Card>

                        {!!vendas.length && (
                            <Table title={() => <p style={{ width: '100%', textAlign: 'center', fontWeight: 'bold', marginBottom: -4 }}>VENDA DE PRODUTOS</p>} pagination={{ pageSize: 10 }} columns={columnSalesWait} dataSource={vendas} size='small' rowKey={(cli) => cli._id} />
                        )}
                        ...
                        <Descriptions layout="vertical" bordered size='small'>
                            <Descriptions.Item label="Total de Vendas de Produtos Pagas" span={3}><Statistic value={totalVendasPay} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
                            <Descriptions.Item label="Total de Vendas de Produtos em Aberto" span={3}><Statistic value={totalVendasWait} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
                        </Descriptions>
                ...
                >
                    ...
                    <Select value={find} style={{ width: '100%' }} onChange={(value) => setFind(value)}>
                        <Option value={1}>Vendas de Produtos do Mês Atual</Option>
                        <Option value={2}>Vendas de Produtos por Mês</Option>
                        <Option value={3}>Vendas de Produtos Anual</Option>
                        <Option value={4}>Vendas de Produtos do Mês Atual - Clientes</Option>
                        <Option value={5}>Vendas de Produtos por Mês - Clientes</Option>
                        <Option value={6}>Vendas de Produtos Anual - Clientes</Option>
                        <Option value={7}>Vendas de Produtos do Mês Atual - Vendedor</Option>
                        <Option value={8}>Vendas de Produtos por Mês - Vendedor</Option>
                        <Option value={9}>Vendas de Produtos Anual - Vendedor</Option>
                    </Select>
                ...
                >
                    ...
                    {!!produtos.length && (
                        <>
                            ...
                            <Table pagination={{ pageSize: 10 }} columns={columnsProduct} dataSource={produtos} size='small' rowKey={(prod) => prod._id} />
