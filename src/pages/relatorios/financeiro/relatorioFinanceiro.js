...
import { Button, Table, Modal, Select, Card, Divider, Row, Statistic, Descriptions, Icon } from 'antd';
...
function warning(title, message) {
  ...
  async function sendAdvancedFind() {
    ...
    setLoading(true);
    await api.post('/report/fluxCashier', {
      find: typeAdvandcedFind, mes: mes, ano: ano
    }).then(response => {
  ...
  function printer() {
    ...
    mywindow.document.write(`
          ...
          <button class="btn-print" onclick="window.print()">IMPRIMIR</button>
            <div class="content">
              ...
              <div class="header-page">
                ...
                ${typeAdvandcedFind === 1 ? (`<h2>RELATÓRIO DE FLUXO DE CAIXA DO MES ATUAL</h2>`) : (`<h2>RELATÓRIO DE FLUXO DE CAIXA DO MES DE: ${mes} DE ${ano}</h2>`)}
      ...
      return `<tr>
                        ...
                        ${prod.statusPay === 'pay' ? (`<td class="pay2">Sim</td>`) : (`<td class="pay2">Não</td>`)}
                      </tr>`
    }).join('')}
                </tbody>
              </table>
      ...
      return `<tr>
                        ...
                        ${rec.statusPay === 'pay' ? (`<td class="pay2">Sim</td>`) : (`<td class="pay2">Não</td>`)}
                      </tr>`;
    }).join('')}
                </tbody>
              </table>
  ...
  const columnsProducts = [
    ...
    },
    {
      ...
      width: '10%',
      align: 'center',
      render: (valor) => <Statistic valueStyle={{ fontSize: 15.5 }} prefix='R$' precision={2} value={valor} />
    },
    {
      title: 'Situação',
      dataIndex: 'statusPay',
      ...
      render: (value) => <>
        {value === 'pay' && (
          <Button size='small' style={{ width: '100%', backgroundColor: '#4caf50', color: '#fff', fontWeight: 'bold' }} type='link'>Pago</Button>
        )}
        {value === 'wait' && (
          <Button size='small' style={{ width: '100%', backgroundColor: '#ffeb3b', color: '#444', fontWeight: 'bold' }} type='link'>Em Aberto</Button>
        )}
      </>,
      width: '11%'
    }
    ...
    <div style={{ height: '100%' }}>
      ...
      <div style={{ marginTop: 15, overflow: 'hidden' }}>
        ...
        <div style={{ marginBottom: 10, width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Card size='small'>
            ...
            {typeAdvandcedFind === 2 && (
              <p style={{ marginBottom: -3, marginTop: -1 }}>Período dos Dados: <strong>{`${mes} de ${ano}`}</strong></p>
            )}
          ...
          <Row>
            ...
            <Button icon='search' type='primary' onClick={() => setModalAdvancedFind(true)}>Busca Avançada</Button>
        ...
        {showTables === true && (
          <>
            ...
            <Table columns={columnsProducts} dataSource={produtos} size='small' rowKey={(line) => line._id} />

            <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10 }} bordered={false}>
              <Row>
                <Icon type='tool' style={{ fontSize: 20, color: '#fff', marginRight: 15 }} />
                <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>PRESTAÇÃO DE SERVIÇOS</span>
              </Row>
            </Card>

            <Table columns={columnsServices} dataSource={servicos} size='small' rowKey={(line) => line._id} />

            <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10 }} bordered={false}>
              <Row>
                <Icon type='rise' style={{ fontSize: 20, color: '#fff', marginRight: 15 }} />
                <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>RECEITAS DIVERSAS</span>
              </Row>
            </Card>

            <Table columns={columnsReceitas} dataSource={receitas} size='small' rowKey={(line) => line._id} />

            <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10 }} bordered={false}>
              <Row>
                <Icon type='fall' style={{ fontSize: 20, color: '#fff', marginRight: 15 }} />
                <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>DESPESAS DIVERSAS</span>
              </Row>
            </Card>

            <Table columns={columnsReceitas} dataSource={despesas} size='small' rowKey={(line) => line._id} />
            ...
            <Descriptions bordered size='small'>
              <Descriptions.Item span={3} label='Total de Venda de Produtos'><Statistic value={valorProdutos} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
              <Descriptions.Item span={3} label='Total de Prestação de Serviços'><Statistic value={valorServicos} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
              <Descriptions.Item span={3} label='Total das Receitas Diversas'><Statistic value={valorReceitas} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
              <Descriptions.Item span={3} label='Total das Despesas Diversas'><Statistic value={valorDespesas} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
              <Descriptions.Item span={3} label='Saldo Atual'><Statistic value={valorProdutos + valorServicos + valorReceitas - valorDespesas} precision={2} prefix='R$' valueStyle={{ fontSize: 15.5 }} /></Descriptions.Item>
            </Descriptions>
          </>
        )}
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="submit" icon='search' type="primary" loading={loading} onClick={() => sendAdvancedFind()}>
            Buscar
                    </Button>,
        ]}
      >
        <label>Selecione uma opção:</label>
        <Select value={typeAdvandcedFind} style={{ width: '100%' }} onChange={(value) => setTypeAdvancedFind(value)}>
