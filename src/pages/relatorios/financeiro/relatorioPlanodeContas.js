...
import { Icon, Button, Table, Modal, Select, Card, Divider, Row, Statistic, Spin, Descriptions } from 'antd';
...
export default function RelatorioPlanodeContas() {
  ...
  async function findMesGerado() {
    setSpinner(true);
    await api.get('/balancete/lastGer').then(response => {
  ...
  async function gerarBalacete() {
    setLoading(true);
    await api.post('/report/createBalancete', {
      mes: mes, ano: ano
    }).then(response => {
      ...
      <Spin spinning={spinner} size='large'>
        ...
        <Row>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }}>
            ...
            <Card size='small'>
              {!lastGerado.length && (
                <p style={{ marginBottom: -3, marginTop: -1 }}>Ainda não existe balancetes gerados</p>
              )}
              {!!lastGerado.length && (
                <p style={{ marginBottom: -3, marginTop: -1 }}>Último balancete gerado: <strong>{`${lastGerado[0].month} de ${lastGerado[0].year}`}</strong></p>
              )}
            </Card>
        ...
        >
          ...
          <p style={{ width: '100%', textAlign: 'center', marginTop: 10, color: '#444', }}>Período a ser gerado: <strong>{`${mes} de ${ano}`}</strong></p>
