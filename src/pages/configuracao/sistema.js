...
export default function DadosSistema() {
  ...
  async function updatePrinter() {
    ...
    success(
      "Sucesso",
      "Padrão de Impressão salvo com sucesso, reinicie o sistema"
    );
  }

  async function findGlobals() {
    api
      .get("/organization/findConfigsGlobal")
      .then((response) => {
  ...
  async function updateConfigsGlobal() {
    setLoadingIcms(true);
    api
      .post("/organization/configsGlobal", {
    ...
    <>
      <Header>
        <p style={{ fontWeight: "bold", marginBottom: -0.01, fontSize: 18 }}>
          <Icon type="desktop" style={{ fontSize: 20 }} /> CONFIGURAÇÕES DO
          SISTEMA
        </p>
