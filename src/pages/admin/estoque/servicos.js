...
export default class GerenciaServicos extends Component {
  ...
  sendUpdateService = async () => {
    ...
    await api
      .put(`/admin/changeServiceInfo/${this.state.idService}`, {
  ...
  sendActive = async () => {
    this.setState({ spinner: true });
    await api
      .put(`/admin/activeService/${this.state.idService}`, {
          ...
          <>
            <Tooltip placement="top" title="Editar">
              <Button
                ...
                size="small"
                style={{ marginRight: 5 }}
                onClick={() => this.handleEditService(id)}
              />
            </Tooltip>
          </>
        ),
        ...
        <Modal
          ...
          footer={[
            <Button
              key="back"
              type="default"
              onClick={() => this.setState({ modalService: false })}
            >
              Cancelar
            </Button>,
            <Button
