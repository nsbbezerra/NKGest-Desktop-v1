...
import { Icon, Button, Spin, Card, Modal, Select, TreeSelect, Divider, Tabs, Drawer, Row, Table, Tooltip, Statistic, Input, Descriptions } from 'antd';
...
export default function GerVendas() {
    ...
    async function finder() {
        setLoading(true);
        await api.post('/admin/findAllSalesAndOrders', {
            find: find, client: clientId, mes: mes, ano: ano, number: number
        }).then(response => {
    ...
    async function delVenda() {
        ...
        await api.delete(`/orders/cancelSale/${vendaToDel}`).then(response => {
    ...
    async function delOrdem() {
        ...
        await api.delete(`/orders/cancelOrdem/${ordemToDel}`).then(response => {
            success('Sucesso', response.data.message);
    ...
    async function handlePrintSale(id) {
        const result = await vendas.find(obj => obj._id === id);
        const address = await enderecos.find(obj => obj.client._id === result.client._id);
        ...
        },
        {
            ...
            render: (id) => <>
                <Tooltip placement='top' title='Imprimir'>
                    <Button style={{ marginRight: 5 }} icon='printer' type='default' shape='circle' size='small' onClick={() => handlePrintSale(id)} />
                </Tooltip>
                <Tooltip placement='top' title='Vizualizar Informações'>
                    <Button style={{ marginRight: 5 }} icon='zoom-in' type='primary' shape='circle' size='small' onClick={() => viewInfoSalePay(id)} />
                </Tooltip>
                <Tooltip placement='top' title='Cancelar Venda'>
                    <Button icon='close' type='danger' shape='circle' size='small' onClick={() => handleDelVenda(id)} />
                </Tooltip>
            </>,
            width: '8%',
            align: 'center'
    ...
    const columnServicesPay = [
        ...
        },
        {
            ...
            render: (id) => <>
                <Tooltip placement='top' title='Imprimir'>
                    <Button style={{ marginRight: 5 }} icon='printer' type='default' shape='circle' size='small' onClick={() => handlePrintOrder(id)} />
                </Tooltip>
                <Tooltip placement='top' title='Vizualizar Informações'>
                    <Button style={{ marginRight: 5 }} icon='zoom-in' type='primary' shape='circle' size='small' onClick={() => viewInfoServicesPay(id)} />
                </Tooltip>
                <Tooltip placement='top' title='Cancelar Ordem'>
                    <Button icon='close' type='danger' shape='circle' size='small' onClick={() => handleDelOrdem(id)} />
                </Tooltip>
            </>,
            width: '8%',
            align: 'center'
    ...
    return (
        <>
            ...
            <Header>
                <p style={{ fontWeight: 'bold', marginBottom: -.01, fontSize: 18 }}><Icon type='shopping' style={{ fontSize: 20 }} /> GERENCIAR VENDAS / ORDENS DE SERVIÇO</p>
                <Link style={{ position: 'absolute', right: 0 }} to='/'><Button type='danger' shape='circle' icon='close' size='small' /></Link>
            </Header>

            <Spin spinning={spinner} size='large'>

                <Button style={{ position: 'absolute', right: 0, top: 0, zIndex: 200 }} icon='search' type='primary' onClick={() => setModalSearch(true)}>Busca Avançada</Button>
                ...
                <Tabs defaultActiveKey="1" type='card' style={{ marginTop: 15 }}>
                    ...
                    <TabPane tab={
                        ...
                        <Table pagination={{ pageSize: 10 }} columns={columnSalesPay} dataSource={vendas} size='small' rowKey={(cli) => cli._id} />
                    ...
                    <TabPane tab={
                        ...
                        <Table pagination={{ pageSize: 10 }} columns={columnServicesPay} dataSource={ordens} size='small' rowKey={(cli) => cli._id} />
            ...
            <Modal
                ...
                closable={false}
                footer={[
                    <Button key="back" icon='close' type='danger' onClick={() => setModalDelVenda(false)}>
                        Não
                        </Button>,
                    <Button key="submit" icon='check' type="primary" loading={loadingDelSale} onClick={() => delVenda()}>
                        Sim
                        </Button>,
                ]}
            >
                ...
                <p style={{ fontStyle: 'italic' }}><strong>OBS:</strong> Se esta venda tiver uma nota fiscal emitida, ela será encaminhada automaticamente ao cancelamento.</p>
            ...
            <Modal
                ...
                closable={false}
                footer={[
                    <Button key="back" icon='close' type='danger' onClick={() => setModalDelOrdem(false)}>
                        Não
                        </Button>,
                    <Button key="submit" icon='check' type="primary" loading={loadingDelOrder} onClick={() => delOrdem()}>
                        Sim
                    </Button>,
                ]}
            >
                ...
                <p style={{ fontStyle: 'italic' }}><strong>OBS:</strong> Se esta ordem de serviço tiver uma nota fiscal emitida, ela será encaminhada automaticamente ao cancelamento.</p>
            ...
            <Modal
                ...
                title="Imprimir"
                onCancel={() => { setModalPrintSale(false) }}
                footer={false}
                width='30%'
                centered
            >
                ...
                {modalPrintSale === true && (
                    <PrintSale empresa={dados} cliente={clientToPrint} endereco={addressToPrint} venda={orderToPrint} />
                )}
            ...
            >
                ...
                {modalPrintOrder === true && (
                    <PrintOrder empresa={dados} cliente={clientToPrint} endereco={addressToPrint} venda={orderToPrint} />
                )}
            ...
            >
                ...
                <Descriptions layout="vertical" bordered size='small'>
                    <Descriptions.Item label="Cliente" span={3}>{clientView}</Descriptions.Item>
                </Descriptions>

                <Divider>Produtos</Divider>

                <Table pagination={false} columns={columnsProduct} dataSource={orderToPrint.products} size='small' rowKey={(prod) => prod.product} />

                <Divider>Pagamentos</Divider>

                <Table pagination={false} columns={columnsPayment} dataSource={pagamentos} size='small' rowKey={(prod) => prod._id} />
            ...
            >
                ...
                <Table pagination={false} columns={columnsService} dataSource={orderToPrint.services} size='small' rowKey={(prod) => prod.service} />
