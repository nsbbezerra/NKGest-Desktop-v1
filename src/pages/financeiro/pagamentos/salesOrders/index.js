...
export default function VerifyVendas() {
  ...
  async function findPaymentSale(id) {
    setSpinner(true);
    await api
      .get(`/payments/findPaymentsById/${id}`)
      .then((response) => {
  ...
  async function findPaymentOrder(id) {
    setSpinner(true);
    await api
      .get(`/payments/findPaymentsOrdersById/${id}`)
      .then((response) => {
  ...
  async function sendUpdateStatusOrder() {
    setLoadingUpdate(true);
    await api
      .put(`/payments/changePaymentSale/${idPagamentoOrder}`, {
              ...
              {!id.boletoUrl && (
                <>
                  {value === "wait" && (
                    <Tooltip
                      placement="top"
                      title="Clique no Botão ao lado para gerar o boleto"
                    >
                      <Button
                        type="link"
                        size="small"
              ...
              {!opt.boletoUrl && (
                <Tooltip placement="top" title="Gerar Boleto">
                  <Button
                    ...
                    size="small"
                    style={{ marginRight: 5 }}
                    onClick={() => generateBoletoOrder(id)}
                  />
                </Tooltip>
              )}
            </>
  ...
  return (
    <Spin spinning={spinner} size="large">
        ...
        <Card size="small">
          {find === 1 && (
            <p style={{ marginBottom: -3, marginTop: -1 }}>
              Tipo dos dados:{" "}
              <strong>Vendas / Ordens de Serviços Mês Atual</strong>
            </p>
          )}
          {find === 2 && (
            <p style={{ marginBottom: -3, marginTop: -1 }}>
              Tipo dos dados:{" "}
              <strong>{`Vendas / Ordens de Serviços do mês: ${mes} do ano: ${ano}`}</strong>
            </p>
          )}
          {find === 3 && (
            <p style={{ marginBottom: -3, marginTop: -1 }}>
              Tipo dos dados:{" "}
              <strong>Todas as Vendas / Ordens de Serviços</strong>
            </p>
          )}
          {find === 4 && (
            <p style={{ marginBottom: -3, marginTop: -1 }}>
              Tipo dos dados:{" "}
              <strong>{`Vendas / Ordens de Serviços do Cliente: ${clientName}`}</strong>
            </p>
          )}
        </Card>
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            icon="close"
            type="danger"
            onClick={() => setModalPaymentsVendas(false)}
          >
            Fechar
          </Button>,
        ]}
      ...
      >
        <Table
          pagination={{ pageSize: 10 }}
          columns={columnsPaymentSale}
          dataSource={pagamentoVendas}
          rowKey={(pay) => pay._id}
          size="small"
          rowClassName={(record) =>
            moment(new Date(record.dateToPay)) < moment(new Date()) &&
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            icon="close"
            type="danger"
            onClick={() => setModalPaymentsOrdens(false)}
          >
            Fechar
          </Button>,
        ]}
      ...
      >
        <Table
          pagination={{ pageSize: 10 }}
          columns={columnsPaymentOrder}
          dataSource={pagamentoOrdens}
          rowKey={(pay) => pay._id}
          size="small"
        />
      </Modal>
