...
import { Spin, Table, Button, Modal, Card, Select, Divider, TreeSelect, Statistic, Tooltip, Radio, Icon, Descriptions, Row, Col, Input, Tabs } from 'antd';
...
export default function Debits() {
    ...
    async function finder() {
        ...
        setLoading(true);
        setSpinner(true);
        await api.post('/admin/listDebitsAll', {
            find: find, client: clientId
        }).then(response => {
    ...
    async function sendUpdateStatusOrder() {
        setLoadingUpdate(true);
        await api.put(`/payments/changePaymentOrder/${idPagamentoOrder}`, {
    ...
    async function createNewPaymentSale() {
        setLoadingUpdate(true);
        await api.post('/payments/newPaymentParcelVendas', {
            oldSale: saleToUpdate, payments: { paymentId: paymentVenda, total: totalPayVendasValue, parcelas: maxParcelaVendas, statusPayment: statusPaymentFirst }
        }).then(response => {
            success('Sucesso', response.data.message);
            setLoadingUpdate(false);
            setModalFinishPaymentVenda(false);
    ...
    async function createNewPaymentOrder() {
        setLoadingUpdate(true);
        await api.post('/payments/newPaymentParcelOrdens', {
            oldSale: orderToUpdate, payments: { paymentId: paymentOrder, total: totalPayOrdensValue, parcelas: maxParcelaOrder, statusPayment: statusPaymentOrder }
        }).then(response => {
            success('Sucesso', response.data.message);
            setLoadingUpdate(false);
            setModalFinishPaymentOrder(false);
        ...
        },
        {
            ...
            render: (id, opt) => <>
                <Tooltip placement='top' title='Novo Pagamento'>
                    <Button shape='circle' icon="dollar" type='primary' size='small' style={{ marginRight: 5 }} onClick={() => newPaymentSale(id)} />
                </Tooltip>
            </>,
            width: '9%',
            align: 'center'
    ...
    const columnsPaymentOrdens = [
        ...
        },
        {
            ...
            render: (id, opt) => <>
                <Tooltip placement='top' title='Novo Pagamento'>
                    <Button shape='circle' icon="dollar" type='primary' size='small' style={{ marginRight: 5 }} onClick={() => newPaymentOrder(id)} />
                </Tooltip>
            </>,
            width: '9%',
            align: 'center'
        ...
        <Spin spinning={spinner} size='large'>
            ...
            <div style={{ marginBottom: 10, width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Card size='small'>
                    ...
                    {find === 1 && (
                        <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>Todos os Débitos</strong></p>
                    )}
                    {find === 2 && (
                        <p style={{ marginBottom: -3, marginTop: -1 }}>Tipo dos dados: <strong>{`Débitos do cliente: ${clientName}`}</strong></p>
                    )}
            ...
            <Tabs defaultActiveKey="1" type='card' tabPosition='left'>
                ...
                <TabPane tab={
                    ...
                    <Table pagination={{ pageSize: 10 }} columns={columnsPaymentVendas} dataSource={pagamentosVendas} rowKey={(pay) => pay._id} size='small' style={{marginBottom: 10}}/>

                    <Descriptions bordered size='small'>
                        <Descriptions.Item label='TOTAL DOS DÉBITOS' span={3}><Statistic value={resultado} valueStyle={{ fontSize: 15.5 }} prefix='R$' precision={2} /></Descriptions.Item>
                    </Descriptions>
                ...
                <TabPane tab={
                    ...
                    <Table pagination={{ pageSize: 10 }} columns={columnsPaymentOrdens} dataSource={pagamentosOrdens} rowKey={(pay) => pay._id} size='small' style={{marginBottom: 10}}/>
            ...
            <Modal
                ...
                footer={[
                    ...
                    <Button key='submit' icon='check' type='primary' loading={loadingUpdate} onClick={() => createNewPaymentSale()}>
                        Finalizar
                    </Button>,
                ]}
                width='60%'
            ...
            >
                ...
                <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10 }} bordered={false}>
                    <Row>
                        <Icon type='shopping' style={{ fontSize: 20, color: '#fff', marginRight: 15 }} />
                        <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>TOTAL DAS VENDAS</span>
                    </Row>
                </Card>
                ...
                <Row gutter={10}>
                    ...
                    <Col span={24}>
                        ...
                        <label>Valor</label>
                        <Input type='number' addonAfter='R$' size='large' value={totalPayVendasValue} onChange={(e) => setTotalPayVendasValue(parseFloat(e.target.value))} />
                ...
                <Row gutter={10} style={{ marginTop: 10 }}>
                    ...
                    <Col span={12}>
                        ...
                        <Select size='large' value={paymentVenda} style={{ width: '100%' }} onChange={(value) => handlePaymentVenda(value)}>
                    ...
                    <Col span={12}>
                        ...
                        {!!parcelasVendas.length && (
                            <>
                                ...
                                <Select value={maxParcelaVendas} size='large' style={{ width: '100%' }} onChange={(value) => setMaxParcelaVendas(value)}>
            ...
            <Modal
                ...
                footer={[
                    ...
                    <Button key='submit' icon='check' type='primary' loading={loadingUpdate} onClick={() => createNewPaymentOrder()}>
                        Finalizar
                    </Button>,
                ]}
                width='60%'
            ...
            >
                ...
                <Card size='small' style={{ backgroundColor: '#001529', marginBottom: 10, marginTop: 15 }} bordered={false}>
                    <Row>
                        <Icon type='file-sync' style={{ fontSize: 20, color: '#fff', marginRight: 15 }} />
                        <span style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>TOTAL DAS ORDENS DE SERVIÇO</span>
                    </Row>
                </Card>
                ...
                <Row gutter={10}>
                    ...
                    <Col span={24}>
                        ...
                        <label>Valor</label>
                        <Input type='number' addonAfter='R$' size='large' value={totalPayOrdensValue} onChange={(e) => setTotalPayOrdensValue(parseFloat(e.target.value))} />
                ...
                <Row gutter={10} style={{ marginTop: 10 }}>
                    ...
                    <Col span={12}>
                        ...
                        <label>Forma de Pagamento</label>
                        <Select size='large' value={paymentOrder} style={{ width: '100%' }} onChange={(value) => handlePaymentOrder(value)}>
                            {formaPagamento.map(forma => (
                    ...
                    <Col span={12}>
                        ...
                        {!!parcelasOrder.length && (
                            <>
                                <label>Parcelas</label>
                                <Select value={maxParcelaOrder} size='large' style={{ width: '100%' }} onChange={(value) => setMaxParcelaOrder(value)}>
