...
import { Icon, Button, Table, Tabs, Tooltip, Modal, Input, Row, Col, Select, Switch, Spin } from 'antd';
...
export default class ContasReceber extends Component {
    ...
    handleSendEditInfo = async () => {
        ...
        await api.put(`/financial/editFormaPagamento/${this.state.idForma}`, {
            name: this.state.name, accountBank: this.state.idAccountBank, maxParcela: this.state.maxParcela, intervalDays: this.state.intervalDays, statusPay: this.state.statusPay, boleto: this.state.boleto, firtsPay: this.state.firtsPay, cheque: this.state.cheque, accData: this.state.acrescimo, credito: this.state.credito
        }).then(response => {
    ...
    handleEditInfo = async (id) => {
        ...
        const result = await this.state.formaDePagamento.find(obj => obj._id === id);
    ...
    sendDelForma = async () => {
        ...
        await api.delete(`/financial/delFormaPagamento/${this.state.idForma}`).then(response => {
            this.success('Sucesso', response.data.message);
    ...
    render() {
        ...
        const columns = [
            ...
            },
            {
                ...
                dataIndex: 'boleto',
                key: 'boleto',
                render: (value) => <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                    {value === true && (
            ...
            },
            {
                ...
                render: (id) => <>
                    <Tooltip placement='top' title='Editar Informações'>
                        <Button shape="circle" icon="edit" type='primary' style={{ marginRight: 5 }} size='small' onClick={() => this.handleEditInfo(id)} />
                    </Tooltip>
                    <Tooltip placement='top' title='Excluir'>
                        <Button shape="circle" icon="close" type='danger' size='small' onClick={() => this.handleDelForma(id)} />
                    </Tooltip>
                </>,
                width: '9%',
                align: 'center'
        ...
        return (
            ...
            <div style={{ height: '100%' }}>
                <Header>
                    <p style={{ fontWeight: 'bold', marginBottom: -.01, fontSize: 18 }}><Icon type='dollar' style={{ fontSize: 20 }} /> FORMAS DE PAGAMENTO</p>
                    ...
                    <Tabs defaultActiveKey="1" tabPosition='left' type='card'>
                        ...
                        <TabPane tab="Listagem" key="1">
                            ...
                            <Spin spinning={this.state.spinner} size='large'>
                                ...
                                <Button type='default' icon='redo' onClick={() => this.findFormaPagamento()} style={{ marginBottom: 10 }}>Atualizar</Button>

                                <Table pagination={{ pageSize: 10 }} columns={columns} dataSource={this.state.formaDePagamento} rowKey={(forma) => forma._id} size='small' />
                ...
                <Modal
                    ...
                    footer={[
                        <Button key="back" icon='close' type='danger' onClick={() => this.setState({ modalDel: false })}>
                            Não
                        </Button>,
                        <Button key="submit" icon='check' type="primary" loading={this.state.loading} onClick={() => { this.sendDelForma() }}>
                            Sim
                        </Button>,
                    ]}
                >
                ...
                <Modal
                    ...
                    footer={[
                        <Button key="back" icon='close' type='danger' onClick={() => this.setState({ modalEditInfo: false })}>
                            Cancelar
                        </Button>,
                        <Button key="submit" icon='search' type="primary" loading={this.state.loading} onClick={() => { this.handleSendEditInfo() }}>
                            Salvar
                        </Button>,
                    ]}
                    width='95%'
                >
                    ...
                    <Row gutter={10}>
                        ...
                        <Col span={8}>
                            ...
                            <Select value={this.state.idAccountBank} style={{ width: '100%' }} onChange={(value) => this.setState({ idAccountBank: value })}>
                        ...
                        <Col span={6}>
                            ...
                            <Select value={this.state.statusPay} style={{ width: '100%' }} onChange={(value) => { this.setState({ statusPay: value }) }}>
                    ...
                    <Row gutter={10} style={{ marginTop: 15 }}>
                        ...
                        <Col span={4}>
                            ...
                            <Input type='number' onChange={(e) => this.setState({ maxParcela: e.target.value })} value={this.state.maxParcela} />
                        ...
                        <Col span={4}>
                            ...
                            <Input type='number' onChange={(e) => this.setState({ intervalDays: e.target.value })} value={this.state.intervalDays} />
                        ...
                        <Col span={4}>
                            ...
                            <label style={{ display: 'block', width: '100%' }}>Transferência / Deposito?<span style={{ color: 'red' }}>*</span></label>
                            <Switch
                                checkedChildren={<Icon type="check" />}
                                unCheckedChildren={<Icon type="close" />}
