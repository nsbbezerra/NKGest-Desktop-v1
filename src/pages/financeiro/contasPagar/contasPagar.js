...
export default class ContasPagar extends Component {
  ...
  sendAdvancedFind = async () => {
    ...
    await api
      .post("/financial/findContasPagar", {
  ...
  sendUpdateConta = async () => {
    ...
    await api
      .put(`/financial/changeContasPagar/${this.state.idConta}`, {
  ...
  sendDelete = async () => {
    ...
    await api
      .delete(`/financial/delContasPagar/${this.state.idConta}`)
      .then((response) => {
        this.success("Sucesso", response.data.message);
  ...
  sendChangePayment = async () => {
    ...
    await api
      .put(`/financial/changePaymentContasPagar/${this.state.idConta}`, {
      ...
      <div style={{ height: "100%" }}>
        <Header>
          <p style={{ fontWeight: "bold", marginBottom: -0.01, fontSize: 18 }}>
            <Icon type="fall" style={{ fontSize: 20 }} /> CONTAS A PAGAR
          </p>
          <Link to="/">
