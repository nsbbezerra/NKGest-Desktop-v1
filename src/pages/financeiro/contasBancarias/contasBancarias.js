...
function ListContasBancarias() {
  ...
  async function handleSendEditInfo() {
    ...
    await api
      .put(`/financial/editConta/${idConta}`, {
        conta: contaName,
        saldo: newContaValue,
      })
      .then((response) => {
    ...
    <div style={{ height: "100%" }}>
      <Header>
        <p style={{ fontWeight: "bold", marginBottom: -0.01, fontSize: 18 }}>
          <Icon type="bank" style={{ fontSize: 20 }} /> CONTAS BANCÁRIAS
        </p>
        <Link to="/">
      ...
      >
        <Row gutter={8}>
          <Col span={8}>
            ...
            <Input
              type="text"
              onChange={(e) => setContaName(e.target.value.toUpperCase())}
              value={contaName}
            />
          </Col>
          ...
          <Col span={8}>
            ...
            <Input
              type="number"
              addonAfter="R$"
              onChange={(e) => setNewContaValue(e.target.value)}
              value={newContaValue}
            />
          </Col>
        </Row>
