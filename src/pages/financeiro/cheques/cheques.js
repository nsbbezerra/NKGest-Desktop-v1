...
function EditCheques() {
  ...
  async function editCheque() {
    ...
    await api
      .put(`/financial/editCheques/${chequeId}`, {
        client: clientId,
        number: numero,
        entity: entidade,
        situation: situacao,
  ...
  async function sendUpdateSituation() {
    setLoading(true);
    await api
      .put(`/financial/editChequeSituation/${chequeId}`, {
        situation: situacao,
      })
      .then((response) => {
        success("Sucesso", response.data.message);
  ...
  async function sendDelCheque() {
    ...
    await api
      .delete(`/financial/delCheques/${chequeId}`)
      .then((response) => {
        success("Sucesso", response.data.message);
        setModalDel(false);
        setLoading(false);
  ...
  const columns = [
    {
      ...
      render: (type) => (
        <>
          {type === "prazo" && (
            <Button size="small" style={{ width: "100%" }} type="link">
              À prazo
            </Button>
          )}
          {type === "vista" && (
    ...
    <div style={{ height: "100%" }}>
      <Header>
        <p style={{ fontWeight: "bold", marginBottom: -0.01, fontSize: 18 }}>
          <Icon type="file-search" style={{ fontSize: 20 }} /> CONTROLE DE
          CHEQUES
        </p>
        <Link to="/">
      ...
      >
        ...
        <Row gutter={8} style={{ marginTop: 10 }}>
          ...
          <Col span={6}>
            <label>
              Emitido em:<span style={{ color: "red" }}>*</span>
            </label>
            <DatePicker
              format="DD/MM/YYYY"
              style={{ width: "100%" }}
              showToday={false}
              value={moment(new Date(emissao))}
              onChange={(value) => setEmissao(value)}
            />
          </Col>
          ...
          <Col span={6}>
            ...
            <DatePicker
              ...
              style={{ width: "100%" }}
              showToday={false}
              value={moment(new Date(vencimento))}
              onChange={(value) => setVencimento(value)}
            />
          </Col>
        </Row>
      ...
      >
        ...
        >
          <Radio value="done">
            <Icon type="check" /> Compensado
          </Radio>
          <Radio value="wait">
            <Icon type="hourglass" /> Aguardando
          </Radio>
        </Radio.Group>
      </Modal>
    </div>
