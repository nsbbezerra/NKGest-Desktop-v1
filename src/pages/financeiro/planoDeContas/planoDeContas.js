...
import { Icon, Button, Table, Tabs, Tooltip, Modal, Input, Row, Col, Select, Spin } from 'antd';
...
export default class PlanoDeContas extends Component {
    ...
    handleSendDelete = async () => {
        ...
        await api.delete(`/financial/delPlanoContas/${this.state.idConta}`).then(response => {
    ...
    handleSendEditInfo = async () => {
        ...
        await api.put(`/financial/editPlanoConta/${this.state.idConta}`,{
            plano: this.state.nomeConta, tipo: this.state.tipoMovimento
        }).then(response => {
    ...
    findPlanoContas = async () => {
        ...
        await api.get('/financial/findPlanoContas').then(response => {
    ...
    render() {
        ...
        return (
            <div style={{ height: '100%' }}>
                <Header>
                    <p style={{ fontWeight: 'bold', marginBottom: -.01, fontSize: 18 }}><Icon type='book' style={{ fontSize: 20 }} /> PLANO DE CONTAS</p>
                    ...
                    <Tabs defaultActiveKey="1" tabPosition='left' type='card'>
                        ...
                        <TabPane tab="Listagem" key="1">
                            ...
                            <Spin spinning={this.state.spinner} size='large'>
                                ...
                                <Button type='default' icon='redo' onClick={() => this.findPlanoContas()} style={{ marginBottom: 10 }}>Atualizar</Button>

                                <Table pagination={{ pageSize: 10 }} columns={columns} dataSource={this.state.planoDeContas} rowKey={(plano) => plano._id} size='small' />
                ...
                <Modal
                    ...
                    footer={[
                        ...
                        <Button key="submit" icon='check' type="primary" loading={this.state.loading} onClick={() => { this.handleSendDelete() }}>
                            Sim
                        </Button>,
                    ]}
                >
                ...
                <Modal
                    ...
                    footer={[
                        ...
                        <Button key="submit" icon='save' type="primary" loading={this.state.loading} onClick={() => { this.handleSendEditInfo() }}>
                            Salvar
                        </Button>,
                    ]}
                    width='75%'
                >
                    ...
                    <Row gutter={8}>
                        ...
                        <Col span={18}>
                            ...
                            <Input type='text' onChange={(e) => this.setState({ nomeConta: e.target.value.toUpperCase() })} value={this.state.nomeConta}/>
                        ...
                        <Col span={6}>
                            ...
                            <Select value={this.state.tipoMovimento} style={{ width: '100%', marginBottom: 20 }} onChange={(value) => this.setState({ tipoMovimento: value })}>
