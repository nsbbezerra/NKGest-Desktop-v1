...
export default class NotaFiscalNFCE extends Component {
    ...
    updateProduct = () => {
        ...
        if(this.state.descontoRef <= 0) {
            this.warning('Atenção', 'O desconto total já foi inserido nos itens');
            return false;
        }
    ...
    setAllDescToProduct = async (id) => {
        if(this.state.descontoRef === 0) {
            this.warning('Atenção', 'O valor total do desconto já foi inserido');
            return false;
        }
        const indexArr = await this.state.items.findIndex(obj => obj._id === id);
