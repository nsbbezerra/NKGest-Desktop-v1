...
import { Icon, Button, Row, Col, Input, Table, Card, TreeSelect, Divider, Tooltip, InputNumber, Modal, Select, Spin, Statistic, Descriptions, Popconfirm } from 'antd';
...
export default function Orcamentos() {
    ...
    async function sendFinder() {
        ...
        await api.post('/orders/findOrcaments', {
            type: searchType, funcionario: funcionario, cliente: cliente, data: `${dia}/${mes}/${ano}`, mes: mes2, ano: ano, numberSale: numberSale
        }).then(response => {
    ...
    async function convertToSale() {
        setLoadingSale(true);
        await api.put(`/orders/convertToSale/${ordem._id}`, {
            products: productOrder, valueBruto: totalBruto, valueLiquido: totalLiquid, desconto: desconto, obs: observation
        }).then(response => {
    ...
    async function allClear() {
        await setOrderFim({}); await setTotalLiquid(0); await setTotalBruto(0);
        await setDesconto(0); await setIdFinishedSale(''); await setDesc(0); await setLiquid(0); await setBrut(0); await setPaymentsSale([]); await setSelectedRowKeys([]); await setTotalAllOrders([]); await setTotalFinishAllOrders(0); await setClientToOrcament(''); await setClientToOrcamentName(''); await setDisableConvertOrca(true); await setNewProducts([]); await setDisableCalcOrcament(true); await setObservation(''); await setViewNewProducts(false); await setClienteNomeDesc('');
    }
    ...
    async function handlePrintOrca(id) {
        ...
        if (!ender) {
            warning('Atenção', 'O cliente não tem um endereço cadastrado, impossível imprimir');
            return false;
        }
        await setAddress(ender);
    ...
    async function inputProducts(prods) {
        const info = await {
            quantity: prods.quantity, product: prods.product, name: prods.name, unidade: prods.unidade, valueUnit: parseFloat(prods.valueUnit), valueTotal: parseFloat(prods.valueTotal), code: prods.code, id: shortId.generate()
        };
    ...
    async function saveOrcamentAll() {
        ...
        await api.put('/orders/convertManyToSale', {
            ids: selectedRowKeys, client: clientToOrcament, products: newProducts, statuSale: 'sale', desconto: desconto, valueLiquido: totalLiquid, valueBruto: totalBruto, funcionario: idVendedor, obs: observation, address: address._id
        }).then(response => {
    ...
    async function delOrder(id) {
        setSpinner(true);
        await api.delete(`/orders/cancelSale/${id}`).then(response => {
        ...
        },
        {
            ...
            render: (id) => <>
                <Tooltip placement='top' title='Visualizar'>
                    <Button shape="circle" icon="printer" size='small' style={{ marginRight: 5 }} type='default' onClick={() => handlePrintOrca(id)} />
                </Tooltip>
                <Tooltip placement='top' title='Visualizar'>
                    <Button shape="circle" icon="search" size='small' style={{ marginRight: 5 }} type='primary' onClick={() => handleOrder(id)} />
                </Tooltip>
                <Tooltip placement='top' title='Excluir'>
                    <Popconfirm title='Deseja excluir esta venda?' okText='Sim' cancelText='Não' onConfirm={() => delOrder(id)}>
    ...
    return (
        <>
            <Header>
                <p style={{ fontWeight: 'bold', marginBottom: -.01, fontSize: 18 }}><Icon type='container' style={{ fontSize: 20 }} /> ORÇAMENTO DE VENDAS</p>
            ...
            <div style={{ marginTop: 10 }}>
                ...
                <div style={{ width: '100%', display: 'flex', flexDirection: 'row' }}>
                    <Button type='primary' icon='search' onClick={() => setModalSearch(true)} style={{ marginRight: 10 }}>Busca Avançada</Button>
                    <Button icon='calculator' type='default' onClick={() => calculatorOrcaments()} style={{ marginRight: 10 }} disabled={disableCalcOrcament}>Calcular Orçamentos</Button>
                    <Button icon='check' type='dashed' onClick={() => convertAllinOne()} disabled={disableConverOrca}>Finalizar Orçamentos</Button>
                </div>

                <Spin spinning={spinner} size='large'>

                    <Table rowSelection={rowSelection} pagination={{ pageSize: 10 }} columns={columns} dataSource={orcaments} size='small' style={{ marginTop: 10, marginBottom: 10 }} rowKey={(orca) => orca._id} />

                    <Descriptions layout="vertical" bordered size='small'>
                        <Descriptions.Item label="Valor Total" span={3}>{`R$ ${parseFloat(totalfinishAllOrders.toFixed(2))}`}</Descriptions.Item>
                    </Descriptions>
                    ...
                    <Col span={17} style={{ height: '100%' }}>
                        ...
                        <Spin spinning={spinner} size='large'>
                            ...
                            <Card size='small' style={{ height: '100%' }} bordered={false}>
                                ...
                                <Divider style={{ fontSize: 14, fontWeight: 'bold' }}>PRODUTOS</Divider>

                                <Row>

                                    <Col span={24} style={{ overflow: 'auto' }}>

                                        <Table pagination={{ pageSize: 7 }} columns={columnsProduct} dataSource={ordem.products} size='small' rowKey={(prod) => prod.product} />
                            ...
                            <Card size='small' style={{ height: '100%' }} bordered={false}>
                                ...
                                <Row gutter={8}>
                                    ...
                                    <Col span={24}>
                                        ...
                                        <label>Cliente</label>
                                        <Input type='text' size='large' value={clientToOrcamentName} readOnly />
                                ...
                                <Row>
                                    ...
                                    <Col span={24} style={{ overflow: 'auto' }}>
                                        ...
                                        <Table pagination={{ pageSize: 7 }} columns={columnsProduct} dataSource={newProducts} size='small' rowKey={(prod) => prod.id} />
                        ...
                        <Card size='small' style={{ height: '100%' }} bordered={false}>
                            ...
                            <Row style={{ marginTop: 15 }}>
                                ...
                                <Col span={24}>
                                    ...
                                    <Button size='large' type='primary' icon='check' style={{ width: '100%' }}
                                        loading={loadingOrcament} onClick={() => saveOrcamentAll()}
                                    >Finalizar venda</Button>
            ...
            <Modal
                ...
                footer={[
                    ...
                    <Button key="print" icon='printer' type="primary" onClick={() => handlePrint()}>
                        Imprimir Venda
                    </Button>,
                ]}
                width='80%'
            ...
            >
                ...
                {viewNewProducts === true && (
                    <Table pagination={false} columns={columnsProduct} dataSource={newProducts} size='small' rowKey={(prod) => prod.id} />
                )}

                {viewNewProducts === false && (
                    <Table pagination={false} columns={columnsProduct} dataSource={ordem.products} size='small' rowKey={(prod) => prod.product} />
                )}
            ...
            >
                ...
                {modalPrint === true && (
                    <PrintSale empresa={dados} cliente={clientObj} endereco={address} venda={orderFim} />
                )}
