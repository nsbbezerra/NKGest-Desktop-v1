...
export default function Orcamentos() {
  ...
  async function delOrderOrcament() {
    setLoadingDel(true);
    await api
      .delete(`/orders/delOrderSale/${orderToDel}`)
      .then((response) => {
  ...
  async function sendFinder() {
    ...
    setLoading(true);
    await api
      .post("/orders/findOrcamentsOrders", {
        type: searchType,
  ...
  async function convertToSale() {
    setLoadingSale(true);
    await api
      .put(`/orders/convertOrderToSale/${ordem._id}`, {
  ...
  async function saveOrcamento() {
    setLoadingSave(true);
    await api
      .put(`/orders/saveOrderService/${orderToSave}`, {
        services: servicesUpdate,
        desconto: desconto,
        valueLiquido: totalOrdemLiquid,
        valueBruto: totalOrdem,
  ...
  async function finders() {
    setSpinner(true);
    await api
      .get("/orders/listsOrcamentOrderService")
      .then((response) => {
    ...
    <>
      <Header>
        <p style={{ fontWeight: "bold", marginBottom: -0.01, fontSize: 18 }}>
          <Icon type="container" style={{ fontSize: 20 }} /> ORÇAMENTO DE ORDENS
          DE SERVIÇO
        </p>
        <div
          style={{
      ...
      >
        {modalSendSell === true && (
          <PaymentModule
            price={ordem.valueLiquido}
            idSale={ordem._id}
            brut={ordem.valueBruto}
            desc={ordem.desconto}
      ...
      >
        ...
        >
          ...
          >
            <Icon type="check" style={{ color: "white" }} /> Processo concluído
            com sucesso!
          </p>
        </Card>
      ...
      <Modal
        ...
        centered
      >
        {modalPrint === true && <TemplatePrint id={orderFim._id} />}
      </Modal>
    </>
  );
}
