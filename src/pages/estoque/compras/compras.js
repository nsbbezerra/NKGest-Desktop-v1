...
function ComprasAtualizar() {
  ...
  async function sendSales() {
    ...
    if (qtdCompra === 0) {
      warning(
        "Atenção",
        "O valor de atualização do estoque está definido como: 0"
      );
      return false;
    }
    ...
    await api
      .put(`/stock/sendSales/${produtoID}`, {
        code: codeSku,
        codeUniversal: codeUniv,
        valueCusto: custo + despesas,
        valueDiversos: despesas,
        valueSale: venda,
        estoqueAct: estoqueAtual + qtdCompra,
        cfop: cpof,
        ncm: ncm,
        cest: cest,
        icms: { rate: icms, csosn: icmsCns, origin: icmsOrigem },
        pis: { rate: pis, cst: pisCns, origin: pisOrigem },
  ...
  async function handleProduto(value) {
    ...
    if (result.margeLucro) {
      const resultMarge = await margeLucroData.marge.find(
    ...
    <>
      <Header>
        <p style={{ fontWeight: "bold", marginBottom: -0.01, fontSize: 18 }}>
          <Icon type="plus" style={{ fontSize: 20 }} /> ADICIONAR COMPRA
        </p>
        <Link to="/">
      ...
      >
        <Spin spinning={spinner} size="large">
            ...
            <Col span={10}>
              ...
              >
                {produtos.map((products) => (
                  <TreeNode
                    value={products.codiname}
                    title={products.codiname}
                    key={products._id}
                  />
                ))}
              </TreeSelect>
          ...
          <Row gutter={10}>
            <Col span={8}>
              <label>
                ICMS<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                type="number"
                addonAfter="%"
              ...
              <Select
                ...
                onChange={(value) => setIcmsCns(value)}
              >
                <Option value={"101"}>
            ...
            <Col span={8}>
              ...
              <label>
                PIS Origem<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                value={pisOrigem}
                onChange={(e) => setPisOrigem(e.target.value)}
              />
            </Col>

            <Col span={8}>
              <label>
                COFINS<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                type="number"
                addonAfter="%"
              ...
              <label>
                COFINS Origem<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                value={cofinsOrigem}
                onChange={(e) => setCofinsOrigem(e.target.value)}
              />
            </Col>
          </Row>
          ...
          <Row gutter={10}>
            <Col span={8}>
              <label>
                Valor de custo<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                type="number"
                addonAfter="R$"
            ...
            <Col span={8}>
              <label>
                Valor de venda<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                type="number"
                addonAfter="R$"
          ...
          <Row gutter={10}>
            <Col span={12}>
              <label>
                Quantidade Atual<span style={{ color: "red" }}>*</span>
              </label>
            ...
            <Col span={12}>
              <label>
                Quantidade da Compra<span style={{ color: "red" }}>*</span>
              </label>
              <Input
                type="number"
