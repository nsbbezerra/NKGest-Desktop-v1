...
import { Icon, Row, Button, Col, Input, Tabs, Select, Divider, TreeSelect, Spin, Descriptions, Modal, Card, Radio } from 'antd';
...
function Produtos() {
    ...
    async function FindFornecedor() {
        ...
        await api.get('/register/listFornecedores').then(response => {
    ...
    async function calculatePriceForSale() {
        ...
        let finalPrice = await totalPrice + calcIcms + calcPis + calcCofins + calcIpi + calcIcmsSt + calcFcp + calcFcpSt + calcFcpRet;
    ...
    async function RegisterProduct() {
        ...
        function allClear() {
            setNome(''); setIdFornecedor(''); setNomeFornecedor(''); setUniMedida(''); setDescricao(''); setCodeUniv('SEM GTIN'); setCusto(0); setDespesas(0); setEstoqueAtual(0); setMargeLucro(); setMargeLucroValue(0); setVenda(0); setViewFornecedor({}); setCodeSku(''); setIcms(0); setPis(0); setCofins(0); setIcmsCsosn(''); setPisCns(''); setCofinsCns(''); setNcm(''); setCest(''); setCpof(''); setModeCalc('margemBruta'); setTypCst('simples'); setIcmsSt(0); setIcmsMVA(0); setIcmsModCalc(''); setFcp(0); setFcpSt(0); setFcpStRet(0); setIpiCode(''); setIpiCst(''); setIpi(0); setMargemLiquida(0); setMarkupDivisor(0); setComission(0); setExpenses(0); setPisCofins(0);
        }

        setLoading(true);

        await api.post('/register/produtos', {
            name: nome,
            code: codeSku,
            ...
            active: true,
            cfop: cpof, ncm: ncm, icms: { rate: icms, csosn: icmsCsosn, origin: icmsOrigem, icmsSTRate: icmsSt, icmsMargemValorAddST: icmsMVA, icmsSTModBC: icmsModCalc, fcpRate: fcp, fcpSTRate: fcpSt, fcpRetRate: fcpStRet, ipiCst: ipiCst, ipiRate: ipi, ipiCode: ipiCode }, pis: { rate: pis, cst: pisCns }, cofins: { rate: cofins, cst: cofinsCns }, cest: cest, margeLucro: margeLucroValue, typeCalculate: modeCalc, markupFactor: { factor: markupDivisor, margeLucro: margemLiquida, comission: comission, otherExpenses: expenses }
        }).then(response => {
    ...
    function calculatePriceForSaleMarkup() {
        let Ctv = parseFloat(icms) + parseFloat(pisCofins) + parseFloat(comission) + parseFloat(expenses) + parseFloat(margemLiquida);
    ...
    return (
        <>
            <Header>
                <p style={{ fontWeight: 'bold', marginBottom: -.01, fontSize: 18 }}><Icon type='tags' style={{ fontSize: 20 }} /> CADASTRO DE PRODUTOS</p>
            ...
            <div style={{ marginTop: 10 }}>
                ...
                <Spin spinning={spinner} size='large'>
                    ...
                    <Tabs defaultActive='1' type='card'>
                        <TabPane tab={
                        ...
                        } key="1" style={{ paddingRight: 30, paddingLeft: 30 }}>

                            <Row style={{ marginBottom: 7 }} gutter={10}>

                                <Col span={10}>
                                    <label>Nome do produto<span style={{ color: 'red' }}>*</span></label>
                                    <Input type='text' onChange={(e) => setNome(e.target.value.toUpperCase())} value={nome} />
                                </Col>

                                <Col span={5}>
                                    <label>Código</label>
                                    <Input type='text' readOnly addonAfter={<Button type='link' size='small' onClick={() => generateSku()}>Gerar</Button>} value={codeSku} />
                                </Col>
                            ...
                            <Row gutter={10}>
                                <Col span={24}>
                                    ...
                                    <TextArea rows={4} onChange={(e) => setDescricao(e.target.value.toUpperCase())} value={descricao} />
                                </Col>
                            </Row>
                        ...
                        <TabPane tab={
                        ...
                        } key="2" style={{ paddingRight: 30, paddingLeft: 30 }}>

                            <Divider style={{ fontSize: 15, fontWeight: 'bold' }}>TRIBUTAÇÃO</Divider>

                            <Row gutter={10}>
                                <Col span={24}>
                                    <Card title='ICMS' headStyle={{ fontWeight: 'bold', backgroundColor: '#f8f8f8' }} size='small' extra={<Radio.Group onChange={(e) => setTypCst(e.target.value)} buttonStyle='solid' value={typeCst}>
                                        <Radio.Button value={'simples'}>Simples Nacional</Radio.Button>
                                        <Radio.Button value={'outros'}>Outros</Radio.Button>
                                    </Radio.Group>}>
                                        <Row gutter={10}>
                                            <Col span={8}>
                                                <label>Alíquota ICMS<span style={{ color: 'red' }}>*</span></label>
                                                <Input type='number' addonAfter='%' value={icms} onChange={(e) => setIcms(parseFloat(e.target.value))} />
                                                {typeCst !== 'simples' ? (
                                                    <>
                                                        <label>ICMS: CST<span style={{ color: 'red' }}>*</span></label>
                                                        <Select value={icmsCsosn} style={{ width: '100%' }} onChange={(value) => setIcmsCsosn(value)}>
                                                ...
                                                <Select value={icmsOrigem} style={{ width: '100%', marginBottom: 10 }} onChange={(value) => setIcmsOrigem(value)}>
                                            ...
                                            <Col span={8}>
                                                ...
                                                <Input type='number' addonAfter='%' value={icmsSt} onChange={(e) => setIcmsSt(parseFloat(e.target.value))} />
                                                <label>ST Margem de Valor Adicionada</label>
                                                <Input type='number' addonAfter='%' value={icmsMVA} onChange={(e) => setIcmsMVA(parseFloat(e.target.value))} />
                                                <label>Modalidade de B.C. da ST</label>
                                                <Select value={icmsModCalc} style={{ width: '100%', marginBottom: 10 }} onChange={(value) => setIcmsModCalc(value)}>
                                            ...
                                            <Col span={8}>
                                                ...
                                                <Input type='number' addonAfter='%' value={fcp} onChange={(e) => setFcp(parseFloat(e.target.value))} />
                                                <label>Alíquota FCP ST</label>
                                                <Input type='number' addonAfter='%' value={fcpSt} onChange={(e) => setFcpSt(parseFloat(e.target.value))} />
                                                <label>Alíquota FCP ST Retido</label>
                                                <Input type='number' addonAfter='%' value={fcpStRet} onChange={(e) => setFcpStRet(parseFloat(e.target.value))} />
                                            </Col>
                                        </Row>
                                    </Card>
                                </Col>
                            ...
                            <Row gutter={10} style={{ marginTop: 10 }}>
                                    ...
                                    <Card title='IPI' headStyle={{ fontWeight: 'bold', backgroundColor: '#f8f8f8' }} size='small'>
                                        <Row>
                                            <Col span={12} style={{ paddingRight: 2.5 }}>
                                                ...
                                                <Input type='number' addonAfter='%' value={ipi} onChange={(e) => setIpi(parseFloat(e.target.value))} />
                                            </Col>
                                            <Col span={12} style={{ paddingLeft: 2.5 }}>
                                                <label>IPI Código</label>
                                                <Input value={ipiCode} onChange={(e) => setIpiCode(e.target.value)} />
                                            </Col>
                                        </Row>
                                        <label>IPI CST</label>
                                        <Select value={ipiCst} style={{ width: '100%' }} onChange={(value) => setIpiCst(value)}>
                                ...
                                <Col span={8}>
                                    <Card title='PIS' headStyle={{ fontWeight: 'bold', backgroundColor: '#f8f8f8' }} size='small'>
                                        ...
                                        <Input type='number' addonAfter='%' value={pis} onChange={(e) => setPis(parseFloat(e.target.value))} />
                                        <label>PIS: CST<span style={{ color: 'red' }}>*</span></label>
                                        <Select value={pisCns} style={{ width: '100%' }} onChange={(value) => setPisCns(value)}>
                                ...
                                <Col span={8}>
                                    <Card title='COFINS' headStyle={{ fontWeight: 'bold', backgroundColor: '#f8f8f8' }} size='small'>
                                        ...
                                        <Input type='number' addonAfter='%' value={cofins} onChange={(e) => setCofins(parseFloat(e.target.value))} />
                                        <label>COFINS: CST<span style={{ color: 'red' }}>*</span></label>
                                        <Select value={cofinsCns} style={{ width: '100%' }} onChange={(value) => setCofinsCns(value)}>
                                            {PisCofinsCst.map(list => (
                                                <Option value={list.code} key={list.code}>{list.desc}</Option>
                                            ))}
                                        </Select>
                            ...
                            <Row gutter={10} style={{ marginTop: 10 }}>
                                ...
                                <Col span={8}>
                                    ...
                                    <Input type='number' value={cpof} onChange={(e) => setCpof(e.target.value)} />
                            ...
                            <Divider style={{ fontSize: 15, fontWeight: 'bold' }}>PREÇOS</Divider>

                            <Card title='CALCULADOR DE PREÇO DE VENDA' style={{ marginTop: 10 }} headStyle={{ fontWeight: 'bold', backgroundColor: '#f8f8f8' }} size='small' extra={<Radio.Group onChange={(e) => setModeCalc(e.target.value)} buttonStyle='solid' value={modeCalc}>
                                    ...
                                    <>
                                        <Row gutter={10}>
                                            <Col span={8}>
                                                ...
                                                <Input type='number' addonAfter='R$' value={custo} onChange={(e) => setCusto(parseFloat(e.target.value))} />
                                            </Col>
                                            <Col span={8}>
                                                <label>Valores Diversos</label>
                                                <Input type='number' addonAfter='R$' value={despesas} onChange={(e) => setDespesas(parseFloat(e.target.value))} />
                                            </Col>
                                            <Col span={8}>
                                                <label>Valor de Venda</label>
                                                <Input type='number' addonAfter='R$' value={venda} onChange={(e) => setVenda(e.target.value)} />
                                            </Col>
                                        </Row>
                                        <Divider style={{ fontSize: 15, fontWeight: 'bold' }}>CALCULAR</Divider>
                                        <Row gutter={10}>
                                            <Col span={12}>
                                            ...
                                            <Col span={12}>
                                                ...
                                                <Button icon='calculator' type='primary' style={{ width: '100%' }} onClick={() => calculatePriceForSale()}>CALCULAR PREÇO DE VENDA</Button>
                                            </Col>
                                        </Row>
                                    </>
                                )}
                                    ...
                                    <>
                                        ...
                                        <Row gutter={10} style={{ marginBottom: 7 }}>
                                            ...
                                            <Col span={4}>
                                                ...
                                                <Input type='number' value={comission} onChange={(e) => setComission(parseFloat(e.target.value))} addonAfter='%' />
                                            </Col>
                                            <Col span={6}>
                                                <label>Despesas Administrativas<span style={{ color: 'red' }}>*</span></label>
                                                <Input type='number' value={expenses} onChange={(e) => setExpenses(parseFloat(e.target.value))} addonAfter='%' />
                                            </Col>
                                            <Col span={6}>
                                                <label>Margem Desejada (Líquida)<span style={{ color: 'red' }}>*</span></label>
                                                <Input type='number' value={margemLiquida} onChange={(e) => setMargemLiquida(parseFloat(e.target.value))} addonAfter='%' />
                                            </Col>
                                        </Row>
                                        <Row gutter={10}>
                                            <Col span={12}>
                                            ...
                                            <Col span={12}>
                                                <label style={{ color: 'transparent' }}>Nova Quantidade</label>
                                                <Button icon='calculator' type='primary' style={{ width: '100%' }} onClick={() => calculatePriceForSaleMarkup()}>CALCULAR PREÇO DE VENDA</Button>
                                            </Col>
                                        </Row>
                                    </>
                                )}
                        ...
                        <TabPane tab={
                        ...
                        } key="4" style={{ paddingRight: 30, paddingLeft: 30 }}>
                            ...
                            <Row gutter={10}>
                                ...
                                <Col span={8}>
                                    ...
                                    <Input type='number' onChange={(e) => setEstoqueAtual(e.target.value)} value={estoqueAtual} size='large' />
                                ...
                                <Col span={16}>
                                    ...
                                    >
                                        ...
                                        {allFornecedores.map(fornec => (
                                            ...
                                            <TreeNode value={fornec.name} title={fornec.name} key={fornec._id} />
                            ...
                            <Row>
                                ...
                                <Col span={24}>
                                    ...
                                    <Descriptions layout="vertical" bordered size='small'>
                                        <Descriptions.Item label="Nome / Nome Fantasia" span={2}>{viewFornecedor.name}</Descriptions.Item>
                                        <Descriptions.Item label="CPF / CNPJ" span={1}>{viewFornecedor.cpf_cnpj}</Descriptions.Item>
                                        <Descriptions.Item label="Telefone Comercial" span={1}>{viewFornecedor.phoneComercial}</Descriptions.Item>
                                        <Descriptions.Item label="Celular 1" span={1}>{viewFornecedor.celOne}</Descriptions.Item>
                                        <Descriptions.Item label="Celular 2" span={1}>{viewFornecedor.celTwo}</Descriptions.Item>
                                        <Descriptions.Item label="Cidade" span={1}>{viewFornecedor.city}</Descriptions.Item>
                                        <Descriptions.Item label="Estado" span={1}>{viewFornecedor.state}</Descriptions.Item>
                                        <Descriptions.Item label="CEP" span={1}>{viewFornecedor.cep}</Descriptions.Item>
                                    </Descriptions>
                            ...
                            <Button type="primary" icon="save" size='large' loading={loading} onClick={() => RegisterProduct()} style={{ marginBottom: 5 }}>
                                Cadastrar Produto
                            </Button>
